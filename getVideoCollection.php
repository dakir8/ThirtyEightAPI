<?php
    ...
    $sql = "SELECT * FROM `video_collection`;";
	...
	for ($i = 0; $i < sizeof($collections); $i++) {
    	...
    	mysql_query("SET NAMES 'utf8'");
    	mysql_select_db($dbname);
		$sql = "SELECT * FROM `video` WHERE `collection_id` = ".$collections[$i]['collection_id'].";";
		...
		$collections[$i]['videos'] = $videos;
	}
	
	$response = array();
	$response['video_collections'] = $collections;
